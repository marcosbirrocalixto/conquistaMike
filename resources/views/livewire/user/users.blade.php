<div class="p-5 bg-gray-100 border border-gray-300 rounded-md card">
    ...
    <p class="pb-2 font-semibold">Listagem de usuários</p>
    <hr>
    ...
    <div class="mt-2 mb-4 align-items-right">
        ...
        <div class="relative w-full">
            ...
            <input type="text" id="simple-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Pesquise pelo nome e e-mail" required wire:model.live="search"/>
        </div>
        </form>
    </div>
            ...
            <tbody class="text-sm text-left text-gray-500 dark:text-gray-400 p-2">           
                    @foreach ($users as $user)
                        <tr class="odd:bg-gray-200 even:bg-gray-100 p-2">
                            ...
                            <td class="w-3/12 p-2">{{ $user->email }}</td>
                            <td class="w-2/12 text-right p-2">
                                <div class="flex justify-end gap-2">
                                    <!-- Botão Editar -->
                                    <a href="{{ route('users.edit', $user->id) }}" title="Alterar registro">
                                ...
                                <!-- Botão Excluir -->
                                <a href="{{ route('users.delete', $user->id) }}"  title="Excluir registro">
    ...
    <div class="grid grid-cols-2 gap-4 mt-5 items-center card mt-4 border border-gray-300 border-3 rounded">
        ...
        <div class="flex justify-start p-2">
            <img src="{{ asset('assets/images/excel-3-32.png') }}" title="Exporta usuários para Excel" class="size-8" wire:click="export()" style="cursor: pointer;">
            <img src="{{ asset('assets/images/pdf.png') }}" title="Relatório em PDF dos usuários" class="size-8 ml-7" wire:click="gerarPdf()" style="cursor: pointer;">
        </div>
        ...
        <div class="flex justify-end items-center p-2">
            ...
            id="per_page" type="number" wire:model="usersPerPage" min="1" wire:blur="alteraPaginacao()">
    ...
    <div class="card mt-4 border border-gray-300 border-3 p-2">
        @if (isset($usersPerPage))
            {!! $users->appends($usersPerPage)->links() !!}
        @else
            {!! $users->links() !!}
        @endif
    </div>
